...
var invertObj = require("./invert_obj");
...
// manifest.browser contains {'jquery': '@scope/jquery'}
  if (browser && !isString(browser)) {
    // alias contains {'@scope/jquery': 'jquery'}
    var alias = invertObj(browser);
    // update the map based on the browser aliases
    return R.pipe(
      // invert the map, since we want to update the keys
      invertObj,
      // update the keys (which are now values) to aliased versions
      R.mapObj(function (val) {
        // val might be '@scope/jquery' or 'foo'
        // in the first case, we want to alias it to just 'jquery'
        return !!alias[val] ? alias[val] : val;
      }),
      // invert back
      invertObj
    )(pkgMap);
