...
var output_dev_dependencies_wo_peer_deps = {
  "map": {
    "*": {
      "when@^3.4.4": "when@3.4.4",
      "@qubit/moment@^2.8.3": "@qubit/moment@2.8.3",
...
describe("rc.npm", function () {
  ...
  it("generates configuration for an npm based project (Without peerDependencies)", function () {
  ...
  it("generates configuration that includes devDependencies (Without peerDependencies)", function() {
    return rc.npm(path.join(__dirname, "fixture"), undefined, {
      includeDevDeps: true,
      excludePeerDeps: true
    }).then(function (config) {
      expect(config).to.deep.equal(output_dev_dependencies_wo_peer_deps);
    });
  });
