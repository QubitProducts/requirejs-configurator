...
function generate(options, cb) {
  ...
  // Whether to exclude peerDependencies or not
  var excludePeerDeps = options.excludePeerDeps === true;
  ...
  var manifests = collectManifests(dependencies, resolve, readManifest, excludePeerDeps);

  var config = when.try(createConfig, manifests, excludePeerDeps);
...
function createConfig(manifests, excludePeerDeps) {
  return {
    map: makeMap(manifests, excludePeerDeps),
    packages: makePackages(manifests)
  };
}

function collectManifests(dependencies, resolve, readManifest, excludePeerDeps) {
  var moduleManifests = {};
  var add = addPackage.bind(null, moduleManifests, null, resolve, readManifest, excludePeerDeps);
  return when.map(dependencies, add).then(function () {
    return moduleManifests;
  });
}

function addPackage(moduleManifests, basedir, resolve, readManifest, excludePeerDeps, pkg) {
  ...
  return getManifest(resolve, readManifest, pkg, basedir).then(function (manifest) {
    if (manifest) {
      ...
      return when.all(mapObj(function (version, pkg) {
        return addPackage(moduleManifests, manifest._location, resolve, readManifest, excludePeerDeps, uid.create(pkg, version));
      }, allDependencies(manifest, excludePeerDeps)));
    } else {
      delete moduleManifests[pkg];
    }
  });
}
...
function getManifest(resolve, readManifest, pkgUid, basedir) {
  ...
  return resolve(pkg.name, pkg.version, basedir).then(function (pkgPath) {
    if (pkgPath) {    
      return readManifest(pkgPath, pkg.name, pkg.version).then(function (manifest) {
...
function makeMap(manifests, excludePeerDeps) {
  var map = {};
  ...
  mapObj(function (manifest, semverName) {
    ...
    map[pkgUid] = R.mapObj.idx(function (version, dependency) {
      ...
      var depManifest,
      dependencyUid = uid.create(dependency, version);

      if (manifests.hasOwnProperty(dependencyUid)) {
        depManifest = manifests[dependencyUid];
...
function allDependencies(manifest, excludePeerDeps) {
  return R.mixin(excludePeerDeps ? {} : manifest.peerDependencies, manifest.dependencies);
}
